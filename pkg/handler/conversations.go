...
type convAddMessageParams struct {
	channel     string
	threadTs    string
	text        string
	contentType string
...
func (ch *ConversationsHandler) convertMessagesFromSearch(slackMessages []slack.SearchMessage) []Message {
	...
	for _, msg := range slackMessages {
		...
		if !ok && msg.User == "" && msg.Username != "" {
			// Bot messages: use Username as fallback
			userName = msg.Username
			realName = msg.Username
			ok = true
		} else if !ok {
...
func (ch *ConversationsHandler) parseParamsToolAddMessage(ctx context.Context, request mcp.CallToolRequest) (*convAddMessageParams, error) {
...
func (ch *ConversationsHandler) paramFormatChannel(raw string) (string, error) {
	...
	if strings.HasPrefix(raw, "C") || strings.HasPrefix(raw, "G") || strings.HasPrefix(raw, "D") {
	...
	if strings.HasPrefix(raw, "@") {
		...
		if channelID, exists := cms.ChannelsInv[raw]; exists {
			return cms.Channels[channelID].ID, nil
		}
