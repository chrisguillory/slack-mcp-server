...
func NewMCPServer(provider *provider.ApiProvider, logger *zap.Logger) *MCPServer {
	...
	s.AddTool(mcp.NewTool("search_messages",
		...
		mcp.WithString("filter_users_with",
			mcp.Description("Filter messages with a specific user in threads and DMs. Must use explicit format: '@username', 'U1234567890' (user ID), or 'D1234567890' (DM channel ID). Plain usernames without @ are not accepted."),
		),
		mcp.WithString("filter_users_from",
			mcp.Description("Filter messages from a specific user. Must use explicit format: '@username', 'U1234567890' (user ID), or 'D1234567890' (DM channel ID). Plain usernames without @ are not accepted."),
		),
